...
var generateOutputTransactions = function (poolRecipient, recipients, rpcData, poolOptions) {
    ...
    if (rpcData.znode) {
        ...
        //Turn Znode founder's rewards off/on in pool options
        if (poolOptions.useZnodeFounders) {
    ...
    if (rpcData.xnode) {
        ...
        let xnode = rpcData.xnode;

        let testnet = poolOptions.testnet;

        //xcoin founders mainnet and testnet
        let xcoinFounders = {
            mainnet: {
                "aCAgTPgtYcA4EysU4UKC86EQd5cTtHtCcr": 1,
                "aHu897ivzmeFuLNB6956X6gyGeVNHUBRgD": 1,
        ...
        if (testnet) {
            founders = xcoinFounders.testnet;
        } else {
            founders = xcoinFounders.mainnet;
        }

        //satoshis in 1 coin (100000000 = 1 BTC f.e.)
        let denomination = 100000000;
        
        //Turn Xnode founder's rewards off/on in pool options
        if (poolOptions.useZnodeFounders) {
            Object.keys(founders).forEach((founderAddress) => {
                let founderReward = founders[founderAddress] * denomination;
        ...
        if (xnode.payee) {
            let payeeReward = xnode.amount;
            let payeeScript = util.addressToScript(xnode.payee);
